...
class Suggested_Itineraries:
    def __init__(self, user, itinerary):
        ...
        for itinerary_options, option in enumerate(self._user.preferences):
            ...
            if option == 'uber':
                uber_option = Uber(origin=self._itinerary.origin, destination=self._itinerary.destination, date=None,
                                   uber_type=None, transit_mode_type=None, itinerary_index=0)
                self._suggested_options.append(['uber', uber_option])
            if option == 'bus':
                bus_option = Transit(origin=self._itinerary.origin, destination=self._itinerary.destination,
